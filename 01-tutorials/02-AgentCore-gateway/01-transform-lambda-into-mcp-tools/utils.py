...
def get_user_pool_id(pool_name: str, client: any) -> str:
    response: dict[str, any] = client.list_user_pools(
        MaxResults=10
    )
    while response:
...
def get_client_id(pool_id: str, client_name: str, client: any) -> str:
    response: dict[str, any] = client.list_user_pool_clients(
        UserPoolId=pool_id,
        MaxResults=10
    )
    while response:
        for pclient in response.get("UserPoolClients", []):
            if pclient["ClientName"] == client_name:
                return pclient["ClientId"]
        response: dict[str, any] = client.list_user_pool_clients(
            UserPoolId=pool_id,
            MaxResults=10,
            NextToken=response["NextToken"]
...
def get_cognito_data(pool: str):
    session = Session()
    cognito_client = boto3.client('cognito-idp', region_name=session.region_name)
    pool_id = get_user_pool_id(pool, cognito_client)
    client_id = get_client_id(pool_id, "sample-agentcore-gateway-client", cognito_client)
    token = reauthenticate_user(client_id)
    return {
            'pool_id': pool_id,
            'client_id': client_id,
            'bearer_token': token,
            'discovery_url':f"https://cognito-idp.{session.region_name}.amazonaws.com/{pool_id}/.well-known/openid-configuration"
        }
